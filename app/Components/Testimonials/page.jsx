...
const Testimonial = () => {
    ...
    // Safe SSR: only read window after mount
    React.useEffect(() => {
    ...
    const visibleItems = isMobile
        ...
        : testimonials.slice(currentIndex * ITEMS_PER_PAGE, currentIndex * ITEMS_PER_PAGE + ITEMS_PER_PAGE);

    const mobileDotIndex = currentIndex;
    const desktopDotIndex = currentIndex;
                ...
                <div className='flex flex-col md:flex-row md:items-end md:justify-between gap-6'>
                    <div>
                        <h2 className='text-white font-semibold text-4xl md:text-[42px] leading-tight'>
                            Loved by <span className='text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400'>10k+ People</span>
                        </h2>
                    ...
                    <div className='hidden md:flex gap-2'>
                        <button
                            onClick={handlePrev}
                            aria-label="Previous"
                            className='h-10 w-10 rounded-lg bg-neutral-900 border border-neutral-800 flex items-center justify-center cursor-pointer hover:bg-neutral-700 hover:border-neutral-600 transition-all text-white'
                        >
                ...
                <div className="flex md:hidden items-center justify-center mt-6 gap-2">
                    {testimonials.map((_, index) => (
                        <span
                            ...
                            className={`block rounded-full transition-all cursor-pointer ${index === mobileDotIndex
                                ? "w-5 h-2 bg-white"
                                : "w-2 h-2 bg-white/25"
                                }`}
                        />
                ...
                <div className="hidden md:flex items-center justify-center mt-6 gap-2">
                    {Array(totalPages).fill(0).map((_, index) => (
                        <span
                            key={index}
                            onClick={() => setCurrentIndex(index)}
                            className={`block rounded-full transition-all cursor-pointer ${index === desktopDotIndex
                                ? "w-5 h-2 bg-white"
                                : "w-2 h-2 bg-white/25"
                                }`}
                        />
